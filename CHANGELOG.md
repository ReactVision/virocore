...
- **VPS-Lite v1 — geo-anchored persistent AR.** Room/building-scale cloud anchors, discoverable by GPS, with an optional attached mesh for occlusion + physics — self-contained "island" anchors, not a continuous unified map. Four workstreams:
  - **Scan session API.** `startScan()`/`finishScan()` (`VROARSessioniOS`/`VROARSessionARCore`, JNI/ObjC bridged) define their own location frame instead of requiring a pre-placed anchor, and aren't capped to a small radius — a whole room can be scanned. `finishScan()` reuses the existing cloud-anchor host pipeline (`libreactvisioncca`) as its "1-scan degenerate case."
  - **Mesh attach.** `snapshotWorldMeshToFile(locationTransform)` serializes the live world mesh (ARKit LiDAR / ARCore depth) to a file, relative to the scan's own location frame; `loadWorldMeshFromFile(filePath, resolvedTransform)` re-attaches a downloaded mesh asset on resolve, for occlusion + Bullet physics collision.
  - **GPS-tagged cloud anchors.** `resolveCloudAnchor()` now returns the anchor's `resolvedTransform` (previously host-only), needed to reposition an attached mesh on the resolving device.
  - **GPS accuracy gating.** `getEarthTrackingState()` now reports a real `Stopped → Localizing → Enabled/Paused` transition gated on horizontal accuracy (`< 15m`), instead of reporting `Enabled` the instant a tracking provider object exists — gated identically on **both** the ReactVision (`VROARSessioniOS`) and ARCore (`VROCloudAnchorProviderARCore`) geospatial provider paths. `isLocationAccuracyReduced()` surfaces iOS 14+ "Precise Location off" / Android coarse-only-location explicitly, since accuracy never converges in that state.

- **Web platform — WASM/WebGL2 renderer core.** virocore now compiles to WebAssembly (`viro-web.wasm`, `wasm/src/cpp/VROSceneWeb.cpp` + `VROARWeb.cpp`), exposing a flat, handle-based C API (`EMSCRIPTEN_BINDINGS(viro_web)`) covering nodes, geometry (box/sphere/surface/polyline/polygon/custom), materials, textures, text, particles, portals, IBL, background (equirect/skybox), lights, camera, model loading (glTF/VRX), declarative + model animation, and an AR backend (`VROARSessionWeb`, a minimal `VROARSession` fed by pose/anchors injected from JS). Ships as the engine behind the new `@reactvision/viro-web-renderer` npm package; no C API needed for video/audio (handled by uploading decoded `<video>`/`<audio>` frames through the existing texture API). AR's perspective projection is now derived off-axis from the camera's real intrinsics when supplied (principal point + focal length), rather than a fixed centered 60° FOV that only aligned with the camera feed by coincidence.
